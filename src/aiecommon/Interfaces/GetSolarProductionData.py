...
class GetSolarProductionData:
    ...
    def _power_pv_production(self):
        """
        Get the predicted power production from each side of the rooftop using the PVGIS API.

        If the PVGIS API call fails with the provided location, the function tries again with the closest inland point to
        the location.
        
        Returns: A dictionary with keys "rootopSide1" to "rootopSideN", where N is the number of sides in the rooftopSummaryTable,
        and values of the predicted power production for each side in kWh/kWp.
        """
        try:
            return {
                f"rootopSide{idx+1}": get_pvgis_data(
            ...
            closest_points = ClosestInlandPoint(self.location.latitude, self.location.longitude)
            try:
                for point in closest_points.get_points():
                    try:
                        pv_production = {
                            f"rootopSide{idx+1}": get_pvgis_data(
                                point.latitude,
                        ...
                        logging.info(f"Point latitude: {point.latitude}, longitude: {point.longitude} not found")
            except Exception:
                logging.error("API call to PVgis failed. Trying to find the closest inland point also failed!")
                raise AieException(AieException.ROOFTOP_ALGORITHM_FAILED) 
...
def get_pvgis_hourly(
    ...
    surface_tilt: int = 30,
    surface_azimuth: int = 0,
    outputformat: str = "json",
    usehorizon: bool = True,
    userhorizon: str = None,
    pvcalculation: bool = True,
    peakpower: float = 0.001,
    pvtechchoice: str = "crystSi",
    mountingplace: str = "free",
    loss: int = 0,
    trackingtype: int = 0,
    optimal_surface_tilt: bool = False,
    optimalangles: bool = False,
    url: str = URL,
    ...
    *Mapped field names are returned when the map_variables argument is True*
    ...
    params = {
        ...
        "optimalinclination": int(optimalangles),
        "loss": loss,
        "localtime": 1,
    }
    ...
    res = requests.get(f"{url}seriescalc", params=params, timeout=timeout)
...
def _parse_pvgis_hourly_json(src, map_variables):
    ...
    data.index = pd.to_datetime(data["time"], format="%Y%m%d:%H%M", utc=False)
...
def _parse_pvgis_tmy_csv(src):
    ...
    #    time,T2m,RH,G(h),Gb(n),Gd(h),IR(h),WS10m,WD10m,SP
    headers = [h.decode("utf-8").strip() for h in src.readline().split(b",")]
    data = pd.DataFrame(
        [src.readline().split(b",") for _ in range(8760)], columns=headers
    )
    dtidx = data["time"].apply(lambda dt: dt.decode("utf-8"))
    dtidx = pd.to_datetime(dtidx, format="%Y%m%d:%H%M", utc=False)
    data = data.drop("time", axis=1)
...
def get_token(user):
    if user in ["nacho", "dominik"]:
        return "92c55605e56ed224d36d3c9640a65c2deeea6e7c"
    else:
        return "no token found"


def empty_data_validator(response):
    # pass validator
...
"""
use function to convert columns of date
"""


def clean_date(col):
    return pd.Series(col.replace("T", " "))
